...
# gateway
data "huaweicloud_compute_flavors" "prod_pool_normal" {
...
resource "huaweicloud_compute_instance" "prod_pool_normal" {
  name               = "prod-npool-00${count.index}"
  hostname           = "prod-npool-00${count.index}"
  key_pair           = var.prod_ecs_keypair
  system_disk_size   = 40
  image_id           = data.huaweicloud_images_image.default.id
  flavor_id          = data.huaweicloud_compute_flavors.prod_pool_normal.ids[0]
  availability_zone  = data.huaweicloud_availability_zones.default.names[0]
  security_group_ids = [
...
resource "null_resource" "run_ansible" {
  triggers = {
    hosts = join(",", [for instance in huaweicloud_compute_instance.prod_pool_normal : instance.network.0.fixed_ip_v4])
    labels = "--node-label byterum.category=biz --node-label byterum.group=normal --node-label byterum.network=private"
  }
  ...
  depends_on = [ huaweicloud_compute_instance.prod_pool, huaweicloud_compute_instance.prod_pool_normal ]
}
